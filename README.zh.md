...
- [5. API 参考](#5-api-参考)
- [6. 促销规则](#6-促销规则)
- [7. 注意事项（本机环境）](#7-注意事项本机环境)
- [8. 前端 Mock（MSW）](#8-前端-mockmsw)
- [9. 测试与覆盖率](#9-测试与覆盖率)
- [10. 架构演进（P0–P4）](#10-架构演进p0p4)
- [11. 安全加固](#11-安全加固)
...
- 异常处理与 HTTP 语义（400 / 404 / 409 / 429 / 503）
- 单元测试 + 集成测试 + JaCoCo 覆盖率门禁（80%）
...
│           ├── application-dev.yml        # dev profile（本地开启 h2 控制台）
│           ├── application-prod.yml       # prod profile（收窄 Actuator + 独立管理端口）
│           └── db/migration/
...
口令采用 `${VAR:-默认值}` 插值；真实值来自被 git 忽略的根 `.env`（模板见 `.env.example`）。零配置 `docker compose up` 仍可用默认值直接跑。
...
**幂等头（可选）**：`POST /api/orders` 还接受请求头 `Idempotency-Key`（唯一字符串）。同 key 重复提交命中已完成记录→回放首次响应 `200`；并发提交→`409`；业务失败→释放该 key 可安全重试。详见 [§10 P2](#p2--可靠性幂等--resilience4j)。
...
GET /api/orders         # 订单列表（redis profile 下走 Redis 缓存 orders-all）
```
...
make ci                      # = test + coverage-check（模拟 CI 流水线）
```

测试覆盖：下单成功（201）、新用户推导、已满减优先、库存不足（400）、订单不存在（404）、响应契约对齐、幂等分支、限流（429）、Outbox 中继等。

> 运行后端会在 `backend/data/` 生成 H2 文件库（已加入 `.gitignore`，不入库）。本地 dev profile 下可用浏览器访问 **http://localhost:8080/h2-console** 直接查看 `orders` / `inventory` 表。
